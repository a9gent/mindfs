...
## 优化和修复
- 修复 windows 上 agent 进程回收泄露
- 修复 刷新页面后 toolcall 行位置下移
- 恢复输入框中蓝环快捷操作，增加上滑快捷切换项目/会话
- 移除 agent 模型选择框中供应商分组
- 增加删除、重命名、移动文件操作
...
## 优化和修复
- api 供应商一键同步：方便新增模型后一键同步到 agent
- 修复dsh 的 token 用量统计错误
- 修复 acp terminal 错误
- 修复访问 markdown 中项目外部文件链接访问错误
- 移除输入框中的蓝点，避免会话消息发送混乱
- mermaid可放大到全屏预览，手机上能放大缩小
- 会话 token 总用量（在agent 回复下方）
